...
static bool run_app(std::uint32_t width, std::uint32_t height)
{
    ...
    std::array constexpr specialization_map_entry{
        VkSpecializationMapEntry{
            .constantID{0},
            .offset{offsetof(specialization_data, enable_wireframe)},
            .size{sizeof(specialization_data::enable_wireframe)}
        }
    };

    VkSpecializationInfo const specialization_info{
        .mapEntryCount{static_cast<std::uint32_t>(std::size(specialization_map_entry))},
        .pMapEntries{specialization_map_entry.data()},
    ...
    VkPipelineRasterizationStateCreateInfo constexpr rasterization_state{
        ...
        .polygonMode{spec_data.enable_wireframe ? VK_POLYGON_MODE_LINE : VK_POLYGON_MODE_FILL},
